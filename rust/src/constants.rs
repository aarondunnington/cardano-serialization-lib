...
impl Constants {
    ...
    pub fn plutus_default_language_views_encoding(_epoch: Epoch) -> LanguageViewEncoding {
        LanguageViewEncoding::new(hex::decode(PLUTUS_DEFAULT_COST_MODELS[0].2).unwrap())
    }
}
...
mod tests {
    ...
    pub fn cost_model_test() {
        ...
        assert_eq!(
            hex::encode(Constants::plutus_default_cost_models(0).language_views_encoding().bytes()),
